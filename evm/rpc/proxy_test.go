...
func TestProxyKey_SerializesMethodAndQuery(t *testing.T) {
	key := proxyKey("GetBalance", BalanceQuery{AddressedQuery: AddressedQuery{Address: "0xabc"}})
	want := `{"method":"GetBalance","query":{"Id":0,"BlockTag":"","Address":"0xabc"}}`
	if key != want {
...
func TestProxyClient_ContextFactoryUsedForNilContext(t *testing.T) {
	...
	result, err := proxy.bytes(nil, "ChainId", nil, func(ctx context.Context) ([]byte, error) {
		if ctx != factoryCtx {
...
func TestProxyClient_HooksWrapCall(t *testing.T) {
	...
	query := BalanceQuery{AddressedQuery: AddressedQuery{Address: "0xabc"}}
	var order []string
	...
	result, err := proxy.bytes(ctx, "GetBalance", query, func(ctx context.Context) ([]byte, error) {
...
func TestProxyClient_PreprocessErrorSkipsCallAndPostprocess(t *testing.T) {
	...
	result, err := proxy.bytes(context.Background(), "ChainId", nil, func(ctx context.Context) ([]byte, error) {
		called.Store(true)
		return []byte("ok"), nil
	})
